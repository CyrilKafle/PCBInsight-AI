...
- **Landing page** (`docs/index.html`, served via GitHub Pages): dark-mode-first static page with a real design system (`docs/design-system.md` — IBM Plex Sans + JetBrains Mono, teal accent kept distinct from the AI-purple already used in the architecture diagram). Every screenshot is genuine output from a live run against `examples/stm32_usb_dev` (including a real Claude review and a real chat answer) — no mockups.
