    ...
    <div className="rounded-lg border border-neutral-800 bg-neutral-900 p-6">
      ...
      <div className="mt-4 flex h-40 items-end gap-1.5">
        {buckets.map((bucket, index) => (
          <div key={index} className="flex flex-1 flex-col items-center gap-1" title={`${bucket.count} net(s), ${bucket.from.toFixed(1)}–${bucket.to.toFixed(1)} mm`}>
            <span className="text-xs text-neutral-400">{bucket.count || ""}</span>
            <div
              className="w-full rounded-t bg-blue-600"
              style={{ height: `${maxCount ? (bucket.count / maxCount) * 100 : 0}%`, minHeight: bucket.count ? 2 : 0 }}
            />
          </div>
        ))}
      </div>
