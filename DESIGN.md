...
**CLI (DONE, built ahead of Phase 4).** `backend/app/cli.py` — `pcbinsight review <path>` runs the full pipeline (parse → `run_all_checks` → `scoring.score` → optional `generate_review` → `html_report.render`) for a single board, or auto-detects a folder of board subdirectories and batch-processes all of them into `<out>/<board-folder>/report.html` plus a `summary.html` index. Packaged via `backend/pyproject.toml`'s `[project.scripts]` so `pip install -e .` gives a real `pcbinsight` command, not just `python -m app.cli`. Batch mode wraps each board's analysis in a broad `except Exception` so one malformed `.kicad_pcb` logs a warning and gets skipped rather than aborting the whole run — a real robustness requirement once "analyze a folder of boards" is the use case, not a nice-to-have. Built as a complement to the future API, not a replacement — a CLI is what makes this feel like a developer tool an engineer would actually reach for, versus only a web report generator.
